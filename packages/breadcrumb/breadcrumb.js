...
var enrichRouteObject = function (path, isCurrent) {
    ...
    var title = Router.routes[path].options.breadcrumb.title;
    if ('function' === typeof title)
        title = Router.routes[path].options.breadcrumb.title();
...
var getAllParents = function () {
    if (Router.current().route) {
        ...
        var parent = Router.current().route.options.breadcrumb.parent;
        if (parent) {
...
var getParentParent = function (parents) {
    // console.log(parents);
    if (newParent = Router.routes[parents[parents.length - 1].path].options.breadcrumb.parent) {
