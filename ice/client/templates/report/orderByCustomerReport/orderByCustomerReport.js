...
Template.ice_customerReportInsertTemplate.helpers({
...
Template.ice_customerReportGen.helpers({
  data: function () {
    ...
    var orderByCustomer = Meteor.callAsync(id, 'orderByCustomer', self);
    if (!orderByCustomer.ready()) {
      return false;
    }
    return orderByCustomer.result();
  },
  name: function (id) {
  ...
  orders.forEach(function (order) {
    ...
    if (order.paidAmount != undefined) {
      if (customerObj[id].paidAmount == undefined && customerObj[id].outstandingAmount ==
        ...
        customerObj[id]['paidAmount'] = customerObj[id]['paidAmount'] +=
          order.paidAmount;
      }
    }
    order.iceOrderDetail.forEach(function (item) {
      customerObj[id]['total'] == undefined ? customerObj[id]['total'] =
        item.amount : customerObj[id]['total'] = customerObj[id][
          'total'
        ] += item.amount;
...
listCustomerAsTable = function (reduceCustomer) {
  ...
  for (var k in reduceCustomer) {
    ...
    td += '<tr style="border: 1px solid #ddd;">' +
      ...
      '<th style="border: 1px solid #ddd;">' + 'តម្លៃសរុប' + '</th>' +
      '</tr>'
    for (var j in reduceCustomer[k]) {
      if (reduceCustomer[k][j].name != undefined) {
        td += '<tr style="border: 1px solid #ddd;">' +
          '<td style="border: 1px solid #ddd;">' + reduceCustomer[k][j].name +
          '</td>' + '<td style="border: 1px solid #ddd;">' + reduceCustomer[k]
          [j].qty + '</td>' + '<td style="border: 1px solid #ddd;">' +
          formatKh(reduceCustomer[k][j].price) + '</td>' +
          '<td style="border: 1px solid #ddd;">' + formatKh(reduceCustomer[k]
            [j].amount) + '</td>' + '</tr>';
      }
    }
    if (reduceCustomer[k].paidAmount != undefined) {
      td += '<tr>' + '<td colspan="2"><strong>ទឹកប្រាក់សរុប: ' + formatKh(
          reduceCustomer[k].total) + '</strong></td>' +
        '<td colspan="1"><strong> ទឹកប្រាក់បានទទួល: ' + formatKh(
          reduceCustomer[k].paidAmount) + '</strong></td>' +
        '<td colspan="1"><strong>ទឹកប្រាក់ជំពាក់: ' + formatKh(reduceCustomer[
          k].outstandingAmount) + '</strong></td>' + '</tr>';
    } else {
