  ...
  for (var k in oldDoc.groupBy['day' + date].items) { // remove items
    if (oldValue.items[k] != undefined) {
      oldDoc.groupBy['day' + date].items[k] = {
        ...
        amount: oldDoc.groupBy['day' + date].items[k].amount - oldValue.items[
          k].amount,
        discount: oldDoc.groupBy['day' + date].items[k].discount - oldValue
          .items[k].discount
      };
    }
  }
  total = oldDoc.total - oldValue.total;
  oldDoc.groupBy['day' + date].discount = oldDoc.groupBy['day' + date].discount -
    oldValue.discount;
  oldDoc.groupBy['day' + date].total = oldDoc.groupBy['day' + date].total -
    oldValue.total;
  oldDoc.groupBy['day' + date].totalInDollar = oldDoc.groupBy['day' + date].totalInDollar -
    oldValue.totalInDollar;
  oldDoc.total = total;
  ...
  for (var k in oldDoc.groupBy['day' + date].items) {
    if (order.items[k] != undefined) {
      oldDoc.groupBy['day' + date].items[k] = {
        ...
        amount: oldDoc.groupBy['day' + date].items[k].amount + order.items[
          k].amount,
        discount: oldDoc.groupBy['day' + date].items[k].discount + order.items[
          k].discount
      };
    }
  }
